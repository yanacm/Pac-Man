...
import pacMan.Pizza.Modo;
import pacMan.CenarioPadrao;
import pacMan.Elemento;
import pacMan.Texto;
import pacMan.Util;
...
public class JogoCenario extends CenarioPadrao {
	...
	public void carregar() {
		...
		fruta = new Elemento[2];
		
		fruta[0] = new Elemento() {
			@Override
		...
		fruta[0].setAtivo(true);
		
		fruta[1] = new Elemento() {
			@Override
			public void desenha(Graphics2D g) {
			if(!isAtivo())
			return;
		...
		inimigos[2].setAtivo(true);
		inimigos[2].setCor(Color.cyan);
		inimigos[2].setDirecao(Direcao.LESTE);
		inimigos[2].setModo(Pizza.Modo.CACANDO);
		
		inimigos[3] = new Pizza(0, 0, largEl, largEl);
		inimigos[3].setVel(3 + Jogo.nivel);
		inimigos[3].setAtivo(false);
		inimigos[3].setCor(Color.MAGENTA);
		inimigos[3].setDirecao(Direcao.LESTE);
		...
		for (int lin = 0; lin < grade.length; lin++) {
			for (int col = 0; col < grade[0].length; col++) {
				if (grade[lin][col] == Nivel.CN || grade[lin][col] == Nivel.SC) {
					...
					pontoVoltaCol = col;
					pontoVoltaLin = lin;
				}else if (grade[lin][col] == Nivel.FT1) {
					fruta[0].setLargura(largEl);
					fruta[0].setAltura(largEl);
					fruta[0].setPx(converteInidicePosicao(col));
					fruta[0].setPy(converteInidicePosicao(lin));
				}else if (grade[lin][col] == Nivel.FT2) {
					fruta[1].setLargura(largEl);
					fruta[1].setAltura(largEl);
					fruta[1].setPx(converteInidicePosicao(col));
					fruta[1].setPy(converteInidicePosicao(lin));
				}
			}
		}
	}
	...
	//volta pro lugar após reiniciar
	...
	public void atualizar() {
		...
		for(Elemento f:fruta) {
			if (!f.isAtivo() && rand.nextInt(1000) == 5) {
				f.setAtivo(true);
				}
				if (Util.colide(pizza, f)) {
				f.setAtivo(false);
				pontos += 100; //cada fruta vale 100
				}
		}
	...
	private void atualizaDirecaoInimigos(Pizza el) {
		...
		if (el.getModo() == Pizza.Modo.PRESO) {
			...
			// Inverte posicao para fugir qnd tá no modo "escuro"
	...
	private void comePastilha(Elemento el) {
		...
		if (grade[lin][col] == Nivel.CN || grade[lin][col] == Nivel.SC) {
			pontos += grade[lin][col] == Nivel.CN ? 5 : 50; //super comida (verde) vale 50 pontos
			totalPastilha--;
	...
	public void desenhar(Graphics2D g) {
		...
		texto.desenha(g, "PONTOS  " + pontos, 185, 20);
		texto.desenha(g, "Levi e Yana", 170, 520);
